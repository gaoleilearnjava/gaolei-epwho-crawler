...
//        System.out.println("抓取分类成功");
        if (nodes.size() != 0) {
//            System.out.println("正在爬取种类列表");
            //说明是种类列表页面
            ...
            if (page.getUrl().toString().contains("chanpin")) {
                ...
                if (url.contains("_")) {
                    ...
                    if (nextPage < pageNum) {
                        page.addTargetRequest(head + "_p" + nextPage + ".html");
                    }
                } else {
                    if (pageNum >= 3) {
                        String[] strings = url.split("\\.");
                        page.addTargetRequest(strings[0] + "." + strings[1] + "." + strings[2] + "_p2.html");
                    }
                }
            } else {
                //爬取商品详情页
                //判断是哪种商品详情页
                if (page.getHtml().xpath("//*[@class='location']//a[1]/text()") != null) {
                    //第一种(常见类型)
